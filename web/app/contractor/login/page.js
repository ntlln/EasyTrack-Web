...
export default function ContractorLogin() {
    ...
    const handleLogin = async () => {
        ...
        if (pathname === "/contractor/login") {
            ...
            const { data: profile, error: profileError } = await supabase.from('profiles').select(`role_id, user_status_id, profiles_status (status_name)`).eq('id', userId).single();
            ...
            const { data: contractorRole, error: contractorRoleError } = await supabase.from('profiles_roles').select('id').eq('role_name', 'Airline Staff').single();
